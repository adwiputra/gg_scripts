...
factor_dir <- (paste(result_dir,"/factor2/", sep="")) #note that in projection, factor2 is used instead
raster_sink <- paste0(result_dir, "/result")
rate_dir_raw <- "D:/GG_Jambi/process/lumens_dir/SCIENDO/BAU_LUCM/transition_0309/" # INPUT newly defined variable; later will be defined as rate_dir #ADCHECK
rate_dir <- paste0(result_dir, "/transition_filter0309") # containing filtered rate #ADCHECK
...
file_init_lc <- paste0(raster_dir, "landcover_2508/LcJb_15.tif")
pu_file <- paste0(raster_dir, "pu/PU_jambi_fin.tif")
id_oilpalm <- 13 #ADCHECK
adj_coef <- 8/11 #ADCHECK
