...
# 0. INPUTS====
working_directory= "D:/GG_Jambi/process/lumens_dir/trial"
raster_directory= "D:/GG_Jambi/General/data/table/land_requirements/raster_dir/"
table_directory = "D:/GG_Jambi/General/data/table"
description= "GGP"
province= "Jambi"
country= "Indonesia"
admin_attribute= readOGR(dsn = "D:/GG_Jambi/General/data/spatial/vector", layer = "admin_diss") # ADedit
field_attribute= "IDADM" # ADedit
spat_res= 100
dissolve_table = "D:/GG_Jambi/General/data/table/kab_diss.csv"
...
fil_tbl <- c("lut_lu.csv", "contrast_GGjambi.csv", "cstock_jambi.csv", "em_peat.csv")
...
type = 1
# period = yir[nyir]
data = paste0(raster_directory, "/pu/PU_jambi_fin.tif") # filename
description= "pu_ver1" # raster name
attribute_table= "D:/GG_Jambi/General/data/table/pu_lookup.csv" # attribute table location which has columns c("ID", "COUNT", "Legend")
#statusoutput=output table
#passfilenames
...
# additional inputs====
lenuses_tarea <- c(3887455, 920934, 84146) # cellcount for 1,4, 6 classes at pristine state
# generate table to compile DIFA values for each scenarios ADreturn
focal_coverage <- as.character(c(1, 4, 6))
...
# LOOP yearth times/2====
for(lenuses_n in 1:(floor((length(lenuses)/2))+1)){ #ADCHECK=====
  if(lenuses_n !=1){
    # 0.5 EXTRA INPUT VARIABLES====
    scenarios <- c("bauH", "bauAlt", "bauP", "ggp", "exp", "tes") #ADedit
    # block <- c("low", "high", "wet")
  ...
  # ADCHECK!====
  row.names(contab) <- c(0,1:31) # handling temporarily the nodata present in the contrast table
