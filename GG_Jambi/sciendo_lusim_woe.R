...
# ADNote
# The scipt works by:
# 1. Generating skeletonFinal based on the available transitions
# 2. Passing the input parameters via set of functor edits
# 3. Loading previously created file under the same directory as defined by the 'SCIENDO_LUCM_index'
...
# interaction with postgre database is minimal somehow:
# 1. Loading the lookup table for landcover class. stored as varible 'lusim_lc'
# 2. Setting the database parameterization
...
result_dir <- "D:/GG_Jambi/process/lumens_dir/SCIENDO/BAU_LUCM"
...
# INPUTS====
# defining the landuse1.tif file location
raster_dir <- "D:/GG_Jambi/General/data/spatial/raster/"
l1_file <- paste0(raster_dir, "landcover_1012/adjustedNA/lc2015_NAsync.tif")# ADinput
# defining the 'landuse2.tif' file location
l2_file <- paste0(raster_dir, "landcover_1012/adjustedNA/lc2018_NAsync.tif")# ADinput
# defining the 'zone.tif' file location
pu_file <- paste0(raster_dir, "pu_Jambi1012/plan_unit_jambi_final.tif")# ADinput
# AD one batch----
