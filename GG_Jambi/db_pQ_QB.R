...
# 0. INPUTS====
working_directory= "C:/restore+/process/LUMENS/trial"
raster_directory= "C:/restore+/process/LUMENS/data/raster"
table_directory = "C:/restore+/process/LUMENS/data/table"
description= "string"
province= "Jambi"
country= "Indonesia"
admin_attribute= readOGR(dsn = "C:/restore+/process/LUMENS/data/vector", layer = "pu_wm")
field_attribute= "IDADM"
spat_res= 500
dissolve_table = "C:/restore+/process/LUMENS/data/table/pu_wm.csv"

# 0.5 EXTRA INPUT VARIABLES====
scenarios <- c("base", "base_rtrw", "bau", "rtrw") # AD toEdit
...
# 2 Add tabular data: lu_lookup, contrast into database====
DB <- dbConnect(
  driver, dbname=project, host=as.character(pgconf$host), port=as.character(pgconf$port),
  user=as.character(pgconf$user), password=as.character(pgconf$pass)
)
...
if(sc == 1){
  yir <- seq(2000, 2045, by = 5)
} else {
  yir <- seq(2010, 2045, by = 5)
}

gen.att_lu <- function(rs_eman=character(), lookup_tl = "C:/restore+/process/LUMENS/data/table/lu_lookup.csv"){# rs_eman = file location of the raster data; lookup_tl = file location of the lookup
  lookup_tl <- read.csv(lookup_tl, stringsAsFactors = FALSE)
  names(lookup_tl)[1] <- "Value"
...
for(nyir in 1:length(yir)){
  ...
  type = 0
  period = yir[nyir]
  data = paste0(raster_directory, "/", scenarios[sc], "/", scenarios[sc], "_", period, ".tif") # filename
...
# INPUTS
analysis.option<- 2
planning_unit = "Administrative.maps"
lukup_lc = "lu_lookup"
raster.nodata = 14
...
# Loop for couple====
for(lenuses_n in 1:(length(lenuses)/2)){
  
  landuse_1=lenuses[(2*(lenuses_n-1))+1]
  landuse_2=lenuses[(2*(lenuses_n-1))+2]
...
# additional inputs====
lenuses_tarea <- c(5326147, 721956, 1380742) # cellcount for 15, 16, 17 classes at pristine state
# generate table to compile DIFA values for each scenarios ADreturn
lenuses_difa_comp_tb <- data.frame(YEAR = yir, DIFA = 0, SCENARIO = scenarios[sc], stringsAsFactors = FALSE)
...
for(lenuses_n in 1:(length(lenuses)/2)){
  if(lenuses_n !=1){
    scenarios <- c("base", "base_rtrw", "bau", "rtrw")
    # block <- c("low", "high", "wet")
    # for(sc in 1: length(scenarios)){ #ADopen
    sc = 1 # ADtemp
  }
  ...
  planning_unit="Administrative.maps"
  raster.nodata= 14
  focal_coverage <- as.character(c(15, 16, 17))
  edgecon="contrast"
  window.shape=1
  adjacent_only=0
  windowsize=1000
  ...
  for(p in 1:length(fa_class)){
    ...
    for(k in 1:(length(bperiod)-1)){
      ...
      lenuses_difa_comp_tb[lenuses_difa_comp_tb$YEAR == pd_1, "DIFA"] <- AUC.init
      lenuses_difa_comp_tb[lenuses_difa_comp_tb$YEAR == pd_2, "DIFA"] <- AUC.final
  ...
  if(lenuses_n == (length(lenuses)/2)) write.csv(lenuses_difa_comp_tb, file = paste0(quesb_dir, "/difa_", fa_class[p], "_", lenuses_scenario, ".csv"), row.names = FALSE)
