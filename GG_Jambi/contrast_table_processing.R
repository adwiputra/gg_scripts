# GG_Jambi
# Run successfully untuk line 198

wdir<-"D:/GG_Jambi/Ecosystem/biodiv"
input_file<-"D:/GG_Jambi/Ecosystem/biodiv/data/table/data_biodiv_JambiSumsel.csv"
...
if (file.exists("qb_metadata.csv")){
  ...
  subDir<-paste0("QuES-B_Tree_diversity_analysis_",cmeta[1,2],"_",cmeta[1,1])
  dir.create(file.path(wdir,subDir))
  out.dir<-file.path(wdir,subDir)
  dir.create(file.path(wdir,subDir))
  write.csv(cmeta, "qb_metadata.csv", row.names=F)
...
#====open file====
file_raw_data <- read.csv(input_file, sep=",", stringsAsFactors = FALSE)
...
horn.disty <- function(dataset = file_raw_data){
  ...
  #3
  #calculating Morisita-Horn dissimilarity indices among different land use syst.
