# AD
# script to create the tabular version of LULC dynamics
...
z_avail <- function(z_num= l){
  av_tab <- big_freqtab1[big_freqtab1$value > z_num*100 & big_freqtab1$value < (z_num+1)*100, ]
...
# A. LOOPING PER PERIOD=====
# for(p in 1: length(project_time)-1){
# periodic inputs====
tpm_rawDir <- "D:/GG_Jambi/simulateTable/run_ready10-15/" #ADchange
# tpmrawDir contains the minimized transition probability table to be adjusted into complete set
# first step, the rate as output by DINAMICA, the rest, the run_ready
tpm_dir <- "D:/GG_Jambi/simulateTable/run_ready10-15/complete_trans/"#CHGABLE
# tpm_dir <- # directory containing the complete set of unchanged and changes; also where the outputs shall be written
# generate big_freqtab1====

# if(p==1){
in_rst_time <- 2015 #CHGABLE
...
big_freqtab1 <- data.frame(freq(lcxzone_rst), stringsAsFactors = FALSE)
big_freqtab1 <- big_freqtab1[!is.na(big_freqtab1$value),]
# } else{ # ADreturn define calculation scheme for period with no raster file

# }

# generate big_freqtab1 \ends====
# transformation of the tables contained within the tpm_rawDir (using algorithms as defined in SupportingUtilities.R)
# Define unchanged step for all of the zones in the folder====
# require big_freqtab
# was run using the raw raw tables
# tpm_rawDir <- "D:/GG_Jambi/General/data/table/land_requirements/table_dir/run_ready15-20/"
setwd(tpm_rawDir)
...
for(z in 1:n_zones){
  ...
  if(nrow(filterRate) != 0){
    ...
    exIn <- unique(big_freqtab1[big_freqtab1$value >= z*100 & big_freqtab1$value < (z+1)*100, "value"] %% 100)
...
# tpm_rawDir <- "D:/GG_Jambi/General/data/table/land_requirements/table_dir/run_ready15-20/"
# tpm_dir <- "D:/GG_Jambi/General/data/table/land_requirements/table_dir/run_ready15-20/complete_trans/"#CHGABLE
setwd(tpm_rawDir)
dir.create(tpm_dir)
transitionFiles <- list.files(tpm_rawDir, pattern = "^Single_step")
n_zones <- length(transitionFiles)
...
for(l in 1: n_zones){
  ...
  tmp_tpm[tmp_tpm$From. == 27 & tmp_tpm$From. == tmp_tpm$To., "Rate"] <- 1
  ...
  # X.5. Filter the rate based on the 
  
}

# filtering the pre.final rate stored in tpm_dir with the name of Single_step_Plan_=====
...
rate_dir_raw <- tpm_dir
rate_dir <- "D:/GG_Jambi/simulateTable/run_ready15-20/"
...
# filtering the pre.final rate stored in tpm_dir with the name of Single_step_Plan_ \ends=====


# calculation of transition ocurrence====
# complete_trans <- data.frame(z = numeric(), LC1 = numeric(), LC2 = numeric(), areaHA = numeric(), period1 = numeric(), period2 = numeric())

complete_trans <- complete_trans <- data.frame(z = numeric(), LC1 = numeric(), LC2 = numeric(), areaHA = numeric(), stringsAsFactors = FALSE)
colCompleteTrans <- names(complete_trans)
# requires big_freqtab1, 
# looop per zone
for(z in 1:n_zones){
  # define the table which contains the lc ids existed prior the transition
  existedFreq <- big_freqtab1[big_freqtab1$value >= z*100 & big_freqtab1$value < (z+1)*100, ]
  existedFreq$value <-  existedFreq$value - z*100
  # transRateTable
  # creating z_id variable to be pasted in the saving process
  if(z < 10) z_id <- paste0("00000", z) else z_id <- paste0("0000", z)
  # reading the raw rate table
  transRateTable <- read.csv(paste0(rate_dir, "Single_step_Plan_", z_id, ".csv"), stringsAsFactors = FALSE)
  transRateTable <- transRateTable[, 1:3]
  # Checking if any existed lc has transition(s) defined
  inClass <- unique(transRateTable$From.)
  outClass <- setdiff(unique(existedFreq$value), inClass)
  
  if(length(outClass) != 0){ # each outClass is to be assigned unchanged transition
    for(oC in 1:length(outClass)){
      addRow <- data.frame(col1 = outClass[oC], col2 = outClass[oC], col3 = 1, stringsAsFactors = FALSE)
      names(addRow) <- names(transRateTable)
      transRateTable <- rbind(transRateTable, addRow)
    }
  }
  # calculation of unchanged portion of classes which undergo changes
  if(length(inClass) != 0){
    for(iC in 1: length(inClass)){
      uncRate <- 1 - sum(transRateTable[transRateTable$From. == inClass[iC], "Rate"])
      addRow <- data.frame(col1 = inClass[iC], col2 = inClass[iC], col3 = uncRate, stringsAsFactors = FALSE)
      names(addRow) <- names(transRateTable)
      transRateTable <- rbind(transRateTable, addRow)
    }
  }
  transRateTable$areaHa <- 0
  # transRateTable is expected to have sufficient records which comprise changes and persistence
  # loop per unique existing value as defined in existedFreq$value
  for(eV in 1:nrow(existedFreq)){
    classInterest <-  existedFreq$value[eV]
    transRateTable[transRateTable$From. == classInterest, "areaHa"] <- transRateTable[transRateTable$From. == classInterest, "Rate"] * existedFreq[existedFreq$value == classInterest, "count"]
  }
  transRateTable$Zone <- z 
  # Organize table
  transRateTable <- transRateTable[, c("Zone", "From.", "To.", "areaHa")]
  names(transRateTable) <- colCompleteTrans
  complete_trans <- rbind(complete_trans, transRateTable)
}
# generate complete transition matrix which includes the unchanged portion of existing landcover
# based on the tables stored in 'rate_dir'
# read the big_freqtab1 to determine the existing landcover
# check whether the transitions defined (From.) has incorporated the existing landcover: if yes, calculate the persistent; otherwise, all is considered as persistent

library(data.table)
xc <- data.frame(a= c("a", "g", "j", "a"), b = c(2,3,4,5), stringsAsFactors = FALSE)
xc <- data.table(xc)
xc <- xc[, lapply(.SD, sum), by = list(a)]
# 
#   
#   
#   
...
# define the big_freqtab which defines the tstart condition
