# GG Papua
# 1/09/2018 mod: 260918 for BAU-H
# ADCHECK!! to be checked a priori
# Script to integrate the LUMENS framework
...
# 0. INPUTS====
working_directory= "D:/GG_Papua/process/lumens_dir/trial"
raster_directory= "D:/GG_Papua/process/lumens_dir/data/raster"
table_directory = "D:/GG_Papua/process/lumens_dir/data/table"
description= "Historical"
province= "Papua"
country= "Indonesia"
admin_attribute= readOGR(dsn = "D:/GG_Papua/process/lumens_dir/data/vector", layer = "KabPuPapua_diss") # ADedit
field_attribute= "IDADM" # ADedit
spat_res= 250
dissolve_table = "D:/GG_Papua/process/lumens_dir/data/table/kab_diss.csv" 
# 0.5 EXTRA INPUT VARIABLES====
scenarios <- c("bauH", "bauAlt", "bauP", "ggp", "exp", "tes") #ADedit
...
fil_tbl <- c("lut_lu.csv", "contrast_GGpapua.csv", "cstock_papua.csv", "em_peat.csv")
...
if(sc == 1){
  yir <- 1990
  yir <- c(yir, seq(2000, 2050, by = 10))
} else if(sc==6){
  yir <- 1990
  yir <- c(yir, seq(2000, 2015, by = 5))
} else {
  yir <- seq(2015, 2045, by = 5)
}
...
type = 1
# period = yir[nyir]
data = paste0(raster_directory, "/planning_unit_resampled.tif") # filename
description= "pu_ver1" # raster name
attribute_table= "D:/GG_Papua/process/lumens_dir/data/table/planning_unit.csv" # attribute table location which has columns c("ID", "COUNT", "Legend")
#statusoutput=output table
#passfilenames
...
# Split: 1. Run QUES-C; 2. COntinue on QUES-B
# ============================
# 4. Running QUES-B====
# ============================
#ADhere
# solving absence of contrast table====
# DB <- dbConnect(
#   driver, dbname=project, host=as.character(pgconf$host), port=as.character(pgconf$port),
#   user=as.character(pgconf$user), password=as.character(pgconf$pass)
# )
...
# list_of_data_lut<-dbReadTable(DB, c("public", "list_of_data_lut"))
# list_of_data_f<-dbReadTable(DB, c("public", "list_of_data_f"))
# # contrast tables input
# fil_tbl <- c("lu_lookup.csv", "contrast.csv")
# tname <- c("lu_lookup", "contrast")
# td <- 2
# table_name <- tname[td]
# tb_bute= paste0(table_directory, "/", fil_tbl[td])
# table_name <- str_replace_all(string=table_name, pattern=" ", repl=".")
...
# idx_lut<-idx_lut+1
# eval(parse(text=(paste("in_lut", idx_lut, "<-read.table(tb_bute, header=TRUE, sep=',')", sep=""))))
# eval(parse(text=(paste("row.names(in_lut", idx_lut, ") <-as.character(c(2:17))", sep=""))))
# eval(parse(text=(paste("list_of_data_lut<-data.frame(TBL_DATA='in_lut", idx_lut,"', TBL_NAME='", table_name, "', row.names=NULL)", sep=""))))
# 
# InLUT_i <- paste('in_lut', idx_lut, sep="")
# 
# dbWriteTable(DB, "list_of_data_lut", list_of_data_lut, append=TRUE, row.names=FALSE)
# dbWriteTable(DB, InLUT_i, eval(parse(text=(paste(InLUT_i, sep="" )))), append=FALSE, row.names=TRUE)
# 
# #write to csv # rudimentary, removed
# list_of_data_lut<-dbReadTable(DB, c("public", "list_of_data_lut"))
# csv_file<-paste(LUMENS_path_user,"/csv_lookup_table.csv", sep="")
...
# additional inputs====
lenuses_tarea <- c(3058674, 785901, 1029653, 219359, 134187, 155844) # cellcount for 1,4, 6 classes at pristine state
# generate table to compile DIFA values for each scenarios ADreturn
focal_coverage <- as.character(c(1, 2, 5, 7, 19, 20)) #ADCHECK
Farea_string <- character()
for(ow in 1:length(focal_coverage)){
  Farea_string <- c(Farea_string, rep(lut.lc[lut.lc$ID == as.numeric(focal_coverage[ow]), "LC"], length(yir)))
}
lenuses_difa_comp_tb <- data.frame(YEAR = yir, DIFA = 0, SCENARIO = scenarios[sc], FOCAREA = Farea_string, stringsAsFactors = FALSE)
# preserving proj.file
lenuses_proj <- proj.file
lenuses_scenario <- scenarios[sc]
#ADhere

# LOOP yearth times/2====
for(lenuses_n in 2:(floor((length(lenuses)/2))+1)){ #ADCHECK
  if(lenuses_n !=1){
    # 0.5 EXTRA INPUT VARIABLES====
    scenarios <- c("bauH", "bauAlt", "bauP", "ggp", "exp", "tes") #ADedit
    # block <- c("low", "high", "wet")
  ...
  proj.file = lenuses_proj
  # pd_1="HIST_2030"
  # pd_2="GGP_2030"
  if(lenuses_n != (floor((length(lenuses)/2))+1)){
    pd_1 = lenuses[(2*(lenuses_n-1))+1]
    pd_2 = lenuses[(2*(lenuses_n-1))+2]
  } else {
    pd_1 = lenuses[(2*(lenuses_n-1))]
    pd_2 = lenuses[(2*(lenuses_n-1))+1]
  }
  ...
  # ADCHECK!====
  row.names(contab) <- c(0,1:24) # handling temporarily the nodata present in the contrast table
  ...
  for(p in 1:length(fa_class)){
    ...
    for(k in 1:(length(bperiod)-1)){
      ...
      totarea <- lenuses_tarea[p] #ADedit 180718
      ...
      lenuses_difa_comp_tb[lenuses_difa_comp_tb$YEAR == pd_1 & lenuses_difa_comp_tb$FOCAREA == names(fa_class[p]), "DIFA"] <- AUC.init
      lenuses_difa_comp_tb[lenuses_difa_comp_tb$YEAR == pd_2 & lenuses_difa_comp_tb$FOCAREA == names(fa_class[p]), "DIFA"] <- AUC.final
  ...
  if(lenuses_n == (floor((length(lenuses)/2))+1)) write.csv(lenuses_difa_comp_tb, file = paste0(quesb_dir, "/difa_", fa_class[p], "_", lenuses_scenario, ".csv"), row.names = FALSE)
  # removing all of the data
  rm(list = grep("^lenuses", x = ls(), invert = TRUE, value = TRUE))
} # loop for yeareth times/2 ends====

# saving difa_compilation
# write.csv(lenuses_difa_comp_tb, file = paste0(quesb_dir, "/difa_", scenarios[sc], ".csv"), row.names = FALSE)




# } LOOP PER BLOCK ENDS ADopen cancelled====

# } # LOOP PER SCENARIOS END====