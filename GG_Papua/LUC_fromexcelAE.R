...
# filtering NA
AF_count_reshape <- AF_count_reshape %>% filter_at(.vars = vars(PLAN, ADMIN), all_vars(!is.na(.)))
...
# filtering NA
tree_count_reshape <- tree_count_reshape %>% filter_at(.vars = vars(PLAN, ADMIN), all_vars(!is.na(.)))
...
deforHa_reshape <- deforTab_rate %>% select(-ncol(.)) %>% gather(year, annHA, -c(1:5)) %>% filter_at(.vars = vars(PLAN, ADMIN), all_vars(!is.na(.)))
...
# per area classification (in hectares/year)====
degradHa_reshape <- degradTab_rate %>% select(-ncol(.)) %>% gather(year, annHA, -c(1:5)) %>% filter_at(.vars = vars(PLAN, ADMIN), all_vars(!is.na(.)))
